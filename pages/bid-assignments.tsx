    ...
    <div className="max-w-screen">
      ...
      <div className="mt-20 flex flex-col mx-auto justify-center">
        <div className="md:hidden flex flex-col mx-auto justify-center align-center">
          <p className="bg-green-900  text-white pt-1 pb-1 ">Make Money by Helping with Homework</p>
          <table className="rounded">
            <thead>
              <tr className="row justify-center align-middle pt-2 pb-2">
                <th className="col-3 text-center">Assigments</th>
              </tr>
            </thead>
            <tbody className="">
              {assignments.map((assignment, index) => (
                <tr
                  key={assignment.id}
                  className={index % 2 === 0 ? 'bg-blue-100 grid mb-2 shadow mx-3' : 'bg-white grid mb-2 shadow mx-3'}
                  onClick={() => handleNavigation(assignment.id)}
                  style={{ cursor: 'pointer' }}
                >
                  <td className="flexs justify-between">
                    <div className="pl-2 pt-1 col-5 text-xs">{assignment.title}</div>
                    <div className="text-center col-2">Bidding: {assignment.status}</div>
                  </td>
                  <td className="text-center col-2">Due Date: {assignment.dueDate}</td>
                  <td className="text-center">{assignment.budget}</td>
                  <td className="text-center col-md">{assignment.offers.length}</td>
                </tr>
              ))}
            </tbody>
          </table>
